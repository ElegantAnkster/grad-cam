...
if __name__ == '__main__':
    ...
    imagePath = args.imagedir_path
    names = os.listdir(imagePath)
    for name in names:
	    rgb_img = cv2.imread((imagePath + "/"  + name), 1)[:, :, ::-1]
	    ...
	    cv2.imwrite(f'./examples/result/{name}_{args.method}_cam.jpg', cam_image)
	    # cv2.imwrite(f'{name}_{args.method}_gb.jpg', gb)
	    # cv2.imwrite(f'{name}_{args.method}_cam_gb.jpg', cam_gb)
